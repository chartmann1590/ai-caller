...
class SipEngine(
    ...
    private var cseq = 1
    /** Destination user (extension) for the active/pending INVITE. */
    private var dialedUser: String? = null
    /** SDP body retained so INVITE can be resent after 401/407. */
    ...
    suspend fun hangup() {
        ...
        val msg = SipMessage(isRequest = true, method = "BYE", requestUri = byeUri, headers = headers)
        sendSip(msg)
    ...
    private suspend fun sendAck() {
        ...
        val msg = SipMessage(isRequest = true, method = "ACK", requestUri = ackUri, headers = headers)
        sendSip(msg)
    }
    ...
    private suspend fun handleAuthChallenge(msg: SipMessage) {
        ...
        if (authAttempts >= 3) {
            ...
            if (cseqHeader.contains("REGISTER")) {
                ...
                _callState.value = AppCallState.Disconnected("Authentication failed")
            }
            return
        }
        authAttempts++
        ...
        val requestUri = if (method == "INVITE" && dialedUser != null) {
            "sip:${dialedUser}@${config.domain}"
        } else {
            "sip:${config.domain}"
        }
        val body = if (method == "INVITE") pendingInviteSdp else ""

        // Prefer Proxy-Authorization for 407, Authorization for 401
        ...
        // Rebuild a clean request — do NOT copy headers from the 401/407 response
        // (that previously dropped Contact/Expires and leaked WWW-Authenticate).
        val headers = mutableMapOf(
            "Via" to "SIP/2.0/UDP $localIp:$localSipPort;branch=z9hG4bK${UUID.randomUUID().toString().substring(0, 8)};rport",
            "Max-Forwards" to "70",
            "From" to (if (method == "INVITE") {
                "\"${config.displayName ?: config.username}\" <sip:${config.username}@${config.domain}>;tag=$fromTag"
            } else {
                "<sip:${config.username}@${config.domain}>;tag=$fromTag"
            }),
            "To" to (if (method == "INVITE" && dialedUser != null) {
                "<sip:${dialedUser}@${config.domain}>"
            } else {
        ...
        if (method == "REGISTER") {
            headers["Expires"] = config.registrationIntervalSeconds.toString()
        }
        if (method == "INVITE" && body.isNotEmpty()) {
            headers["Content-Type"] = "application/sdp"
        }
        if (isProxy) {
            headers["Proxy-Authorization"] = authVal
        } else {
            headers["Authorization"] = authVal
        }
        // qop is parsed for future auth-int support; digest currently uses RFC 2617 without qop.
        if (!qop.isNullOrBlank()) {
            // no-op: algorithm kept as MD5 (default)
        }

        val resendMsg = SipMessage(
            isRequest = true,
            method = method,
    ...
    /**
     * Discover a non-loopback IPv4 address suitable for Contact/Via/SDP.
     * InetAddress.getLocalHost() often returns 127.0.0.1 on Android, which
     * breaks SIP registration and RTP behind any real network path.
     */
    private fun getLocalIp(): String {
        // 1) UDP connect trick — does not send packets, selects a routable iface
        try {
            DatagramSocket().use { socket ->
                socket.connect(InetAddress.getByName("8.8.8.8"), 53)
                val addr = socket.localAddress?.hostAddress
                if (!addr.isNullOrBlank() && addr != "0.0.0.0" && !addr.startsWith("127.")) {
                    return addr
                }
            }
        } catch (_: Exception) {
