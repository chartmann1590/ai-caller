...
import com.charles.localcallagent.telephony.api.RegistrationState
...
class SipEngine(
    ...
    private suspend fun handleAuthChallenge(msg: SipMessage) {
        ...
        val authVal = SipMessage.computeDigestAuth(username = config.authUsername, realm = realm, password = config.password, method = method, uri = requestUri, nonce = nonce, opaque = opaque, qop = qop)
        val headers = mutableMapOf(
    ...
    private fun extractParameter(header: String, paramName: String): String? {
        val quotedKey = "$paramName=\""
        val quotedIdx = header.indexOf(quotedKey)
        if (quotedIdx != -1) {
            val start = quotedIdx + quotedKey.length
            val end = header.indexOf('"', start)
            if (end == -1) return null
            return header.substring(start, end)
        }
        // Some servers send qop/algorithm unquoted (e.g. qop=auth) despite RFC 2617
        // specifying qop-options as a quoted-string.
        val bareKey = "$paramName="
        val bareIdx = header.indexOf(bareKey)
        if (bareIdx == -1) return null
        val start = bareIdx + bareKey.length
        val end = header.indexOfFirst(start) { it == ',' || it == ';' || it == ' ' }
        return header.substring(start, end)
    }
